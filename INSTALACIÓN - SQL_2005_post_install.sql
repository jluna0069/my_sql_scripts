...
/***********************************************************
*                                                          *
*   Configura el SQL Server 2000 luego de la instalación   *
*                                                          *
*                        SQLAdmins			   *
*							   *
*            Ultima Modificación: 09/11/2004               *
*                                                          *
***********************************************************/

/* NOTAS:                                                                                                      */
/* Ejecutar este script luego de una instalación conforme a las normas del documento de nomenclaturas;         */
/* también puede ejecutarse en cualquier instalación ya operativa de SQL Server.                               */
/* Los Sist.Op. probados son: Win 2000 Server / Win 2000 Advance Server / Win 2003 Standard / Win 2003 Ent.Ed. */
/* Las versiones de SQL Server probadas son: SQL Server 2000 Enterprise Edition                                */

/*
 Pasos que realiza el script:
   . Crea las carpetas de Backups y Reportes
   . Habilita el server para poder ser Target Server
   . Configura los servicios SQLServer y SQLServerAgent en 'Auto restart if it stops unexpectedly'
   . Configura el nivel de auditoría de SQL Server en 'Only Failure'
   . Configura el Error Log de SQL Server para que guarde los últimos 30 booteos
   . Setea en '1' la opción 'show advanced options' del SQL Server
   . Agrega a 'SQLAdmins' como SysAdmin y quita el acceso a 'Administrators' local
   . Pone la base de datos Model en Recovery Mode 'Simple'
   . Agrega los roles standard a la base de datos Model
   . Crea la base de datos DBA y sus objetos:
				Base de datos DBA:
								tablas:		[dbo].[DB_Spaces]
										[dbo].[DBA_ABMJobs]
								sp's:		[dbo].[asigna_permisos_sp]
										[dbo].[asigna_permisos_fn]
										[dbo].[permisos]
										[dbo].[perfiles]
										[dbo].[db_space]
										[dbo].[script_seguridad]
										[dbo].[sp_backup_log]
										[dbo].[sp_genera_login]
										[dbo].[sp_hexadecimal]
										[dbo].[sp_reindex_dinamico]
										[dbo].[sp_script_dbroles]
										[dbo].[sp_script_logins]
										[dbo].[sp_seg_usuarios]
				Base de datos master:
								sp's:		[dbo].[aviso_shutdown]
										[dbo].[sp_who2_dba]
				Base de datos msdb:
								trigger:	[dbo].[auditoria]

   . Hace un restart del servicio SQLServerAgent
   . Presenta la versión instalada de SQL Server
*/
...
**********************************************/
set nocount on
declare @path_var varchar(255), @command varchar(255), @var varchar(255), @id int
declare @carpetas table (id int identity, carp varchar(255))
declare @espacios table (id int identity, esp  int)

select 'CREA LAS CARPETAS DE BACKUPS DE DATOS Y SEGURIDAD'
exec master.dbo.xp_instance_regread 'HKEY_LOCAL_MACHINE', 'SOFTWARE\Microsoft\MSSQLServer\Setup', 'SQLDataRoot', @path_var output
select @command = 'md ' + substring(@path_var,1,2) + '\Backup_full'
exec master.dbo.xp_cmdshell @command
select @command = 'md ' + substring(@path_var,1,2) + '\Backup_log'
exec master.dbo.xp_cmdshell @command
select @command = 'md ' + substring(@path_var,1,2) + '\Backup_full' + '\Backup_sec'
exec master.dbo.xp_cmdshell @command

select 'CARPETAS DE BACKUPS DE DATOS Y SEGURIDAD'
select @command = 'dir ' + substring(@path_var,1,2) + '\*.*'
exec master.dbo.xp_cmdshell @command
select @command = 'dir ' + substring(@path_var,1,2) + '\Backup_full\*.*'
exec master.dbo.xp_cmdshell @command

select 'CREA LA CARPETA DE REPORTES'
select @command = @path_var
insert @carpetas values(substring(@command,1,2))
select @var = right(@command,(len(@command)-3))
while @var <> ''
 begin
   delete @espacios
   insert @espacios select PATINDEX ('%\%',@var) 
   if (select esp from @espacios) <> 0
    begin
      insert @carpetas select left(@var,(select esp from @espacios)-1)
      select @var = (select right(@var,(len(@var)-(select esp from @espacios))))
    end
   else
    begin
      insert @carpetas values(@var)
      select @var = ''
    end
 end
select @id = (select max(id) from @carpetas)
while  @id <> 0
 begin
   select @var = carp from @carpetas where id = @id
   if (PATINDEX ('% %',@var)) <> 0
     update @carpetas set carp = '"' + carp + '"' where id = @id
   select @id = @id - 1
 end
select @id = (select min(id) from @carpetas)
select @command = ''
while  @id <> (select max(id) from @carpetas) + 1
 begin
   select @command = @command + (select carp from @carpetas where id = @id) + '\'
   select @id = @id + 1
 end
select @var = @command
select @command = 'md ' + @command + 'Reportes'
exec master.dbo.xp_cmdshell @command

select 'CARPETA DE REPORTES'
select @command = @var
select @command = 'dir ' + @command + '*.*'
exec master.dbo.xp_cmdshell @command
...
**************************************************************************************************/
set nocount on
declare @path_int int

select 'HABILITA EL SERVER PARA PODER SER TARGET SERVER'
exec master.dbo.xp_instance_regwrite 'HKEY_LOCAL_MACHINE', 'SOFTWARE\Microsoft\MSSQLServer\SQLServerAgent', 'MsxEncryptChannelOptions', REG_DWORD, 1

select 'ESTADO DE LA OPCION QUE HABILITA EL SERVER PARA PODER SER TARGET SERVER'
exec master.dbo.xp_instance_regread 'HKEY_LOCAL_MACHINE', 'SOFTWARE\Microsoft\MSSQLServer\SQLServerAgent', 'MsxEncryptChannelOptions', @path_int output
select 'SQL Server      : ' + (case @path_int when 0 then 'DESACTIVADO' when 1 then 'ACTIVADO' end)
...
select 'CONFIGURA EL "SQL SERVER" Y EL "SQL SERVER AGENT" EN "AUTO RESTART IF IT STOPS UNEXPECTEDLY"'
exec msdb.dbo.sp_set_sqlagent_properties @monitor_autostart = 1, @sqlserver_restart = 1

select 'ESTADO DE LA OPCION "AUTO RESTART IF IT STOPS UNEXPECTEDLY"'
exec master.dbo.xp_instance_regread 'HKEY_LOCAL_MACHINE', 'SOFTWARE\Microsoft\MSSQLServer\SQLServerAgent', 'RestartSQLServer', @path_int output
select 'SQL Server      : ' + (case @path_int when 0 then 'DESACTIVADO' when 1 then 'ACTIVADO' end)
exec master.dbo.xp_instance_regread 'HKEY_LOCAL_MACHINE', 'SOFTWARE\Microsoft\MSSQLServer\SQLServerAgent', 'MonitorAutoStart', @path_int output
select 'SQL Server Agent: ' + (case @path_int when 0 then 'DESACTIVADO' when 1 then 'ACTIVADO' end)
...
select 'CONFIGURA EL NIVEL DE AUDITORÍA DE SQL SERVER EN "ONLY FAILURE"'
exec master.dbo.xp_instance_regwrite 'HKEY_LOCAL_MACHINE', 'SOFTWARE\Microsoft\MSSQLServer\MSSQLServer', 'AuditLevel', REG_DWORD, 2

select 'ESTADO DE LA AUDITORIA DE SQL SERVER'
exec master.dbo.xp_instance_regread 'HKEY_LOCAL_MACHINE', 'SOFTWARE\Microsoft\MSSQLServer\MSSQLServer', 'AuditLevel', @path_int output
select 'Audit Level: ' + (case @path_int when 0 then 'NONE' when 1 then 'SUCCESS' when 2 then 'FAILURE' when 3 then 'ALL' end)
...
select 'CONFIGURA EL ERROR LOG DE SQL SERVER PARA QUE GUARDE LOS ÚLTIMOS 30 BOOTEOS'
exec master.dbo.xp_instance_regwrite 'HKEY_LOCAL_MACHINE', 'SOFTWARE\Microsoft\MSSQLServer\MSSQLServer', 'NumErrorlogs', REG_DWORD, 30

select 'CANTIDAD DE ARCHIVOS QUE GUARDARA EL ERROR LOG DE SQL SERVER'
exec master.dbo.xp_instance_regread 'HKEY_LOCAL_MACHINE', 'SOFTWARE\Microsoft\MSSQLServer\MSSQLServer', 'NumErrorlogs', @path_int output
select 'Cantidad: ' + convert(varchar(255),@path_int)
...
select 'AGREGA A "SQLADMINS" COMO SYSADMIN Y QUITA EL ACCESO A "ADMINISTRATORS" LOCAL'
exec master.dbo.sp_grantlogin        'DISCO\SQLAdmins'
exec master.dbo.sp_addsrvrolemember  'DISCO\SQLAdmins',		sysadmin
exec master.dbo.sp_grantlogin        'DISCO_01\SQLAdmins'
exec master.dbo.sp_addsrvrolemember  'DISCO_01\SQLAdmins',	sysadmin
...
select 'CREA LA BASE DE DATOS DBA Y SUS OBJETOS'
if not exists (select name from master.dbo.sysdatabases where name = 'DBA')
  begin
	declare @ubic varchar(255), @create varchar(255)
	select @ubic = left(filename,(len(filename)-10)) from sysdatabases where name = 'master'
	select @create = 'create database [DBA] on (name = ''DBA_Data'', filename = ''' + @ubic + 'DBA_Data.MDF'', size = 80) log on (name = ''DBA_Log'', filename = ''' + @ubic + 'DBA_Log.LDF'', size = 20)'
	exec (@create)
  end
go

exec sp_dboption 'DBA', 'autoclose', 'false'
go
exec sp_dboption 'DBA', 'bulkcopy', 'false'
go
exec sp_dboption 'DBA', 'trunc. log', 'true'
go
exec sp_dboption 'DBA', 'torn page detectio', 'true'
go
exec sp_dboption 'DBA', 'read only', 'false'
go
exec sp_dboption 'DBA', 'dbo use', 'false'
go
exec sp_dboption 'DBA', 'single', 'false'
go
exec sp_dboption 'DBA', 'autoshrink', 'false'
go
exec sp_dboption 'DBA', 'ANSI null default', 'false'
go
exec sp_dboption 'DBA', 'recursive triggers', 'false'
go
exec sp_dboption 'DBA', 'ANSI nulls', 'false'
go
exec sp_dboption 'DBA', 'concat null yields null', 'false'
go
exec sp_dboption 'DBA', 'cursor close on commit', 'false'
go
exec sp_dboption 'DBA', 'default to local cursor', 'false'
go
exec sp_dboption 'DBA', 'quoted identifier', 'false'
go
exec sp_dboption 'DBA', 'ANSI warnings', 'false'
go
exec sp_dboption 'DBA', 'auto create statistics', 'true'
go
exec sp_dboption 'DBA', 'auto update statistics', 'true'
go

use DBA
go

if (select suser_sname(sid) from master..sysdatabases where name = 'DBA') <> 'sa'
 exec sp_changedbowner 'sa'
go

if not exists (select name from DBA.dbo.sysobjects where name = 'DB_Spaces')
...
if exists (select name from dbo.sysobjects where name = 'DBA_ABMJobs')
   drop table DBA_ABMJobs
go

create table [dbo].[DBA_ABMJobs] (
	[Id] [int] IDENTITY (1, 1) constraint [PK_DBA_ABMJOBS] primary key clustered ,
...
if exists (select name from DBA.dbo.sysobjects where name = 'asigna_permisos_sp')
...
if exists (select name from DBA.dbo.sysobjects where name = 'asigna_permisos_fn')
...
declare @permiso varchar(500), @cont2 int
insert @databases(dbname) select '[' + name  + ']' from master..sysdatabases
 where name not in ('master', 'model', 'msdb', 'tempdb', 'northwind', 'pubs', 'distribution', 'DBA', 'system', 'mantenimiento')
  and status in (4, 8, 12, 16, 20, 24, 28, 4194304, 4194308, 4194312, 4194316, 4194320, 4194328, 4194332) order by name desc
select @cont1 = max(nro) from @databases
while  @cont1 <> 0
  begin
...
if exists (select name from DBA.dbo.sysobjects where name = 'permisos')
...
if exists (select name from DBA.dbo.sysobjects where name = 'perfiles')
...
if exists (select name from DBA.dbo.sysobjects where name = 'db_space')
...
fetch basecursor into @aux
 while @@fetch_status = 0
  begin
	 ...
	 truncate table #pivot
	 insert DBA.dbo.DB_Spaces (database_name , database_size , unallocated_space)
	 ...
	 where database_name = @aux and 
	  point = (select max(point) from DBA.dbo.DB_Spaces)
...
execute DBA.dbo.sp_script_dbroles
print ''
print '----------------------------------------------------------------------------------------------------------------------'
print ''
execute DBA.dbo.sp_script_logins
...
if exists (select * from DBA.dbo.sysobjects where name = 'logins' and type = 'U') drop table DBA.dbo.logins
create table DBA.dbo.logins (id int identity, sid varbinary (255), login sysname)
insert DBA.dbo.logins select sid, name from master.dbo.syslogins where name not like 'sa' order by name desc
select @contlg = max(id) from DBA.dbo.logins
while @contlg > 0
begin
	select @login_name = login from DBA.dbo.logins where id = @contlg
	select @pass = null, @defdata = null, @defleng = null, @status = null, @sid = null

	exec DBA.dbo.sp_genera_login @pass out, @defleng out, @defdata out, @status out, @sid out, @login_name
...
if exists (select name from master.dbo.sysobjects where name = 'sp_who2_DBA')
	drop procedure dbo.sp_who2_DBA
go
...
CREATE PROCEDURE sp_who2_DBA  --- 2006/05/03 10:16
...
if exists (select name from sysobjects where name = 'AUDITORIA' and type = 'TR')
   drop trigger AUDITORIA
go

create trigger [AUDITORIA] ON [dbo].[sysjobs]
for insert, update, delete
as
declare @message varchar(255), @jobname varchar(200)
if ((select count(*) from deleted) >0 and (select count(*) from inserted)<1)
begin
	select @jobname = [name] from deleted 
	insert DBA.dbo.DBA_ABMJobs (Servidor, Job, Tipo, Usuario) values (@@servername, @jobname, 'ELIMINADO',SYSTEM_USER)
end
else
begin
	select @jobname = [name] from inserted 
	insert DBA.dbo.DBA_ABMJobs (Servidor, Job, Tipo, Usuario) values (@@servername, @jobname, 'MODIFICADO',SYSTEM_USER)
end
go
...
select 'LISTA LA BASE DE DATOS DBA, SUS OBJETOS, Y LOS OBJETOS CREADOS EN LAS BASES DE DATOS MASTER Y MSDB'
select name as 'Base de datos' from master.dbo.sysdatabases where name in ('DBA', 'master', 'msdb')
(select name as 'Objetos' from DBA.dbo.sysobjects where name in
	(
	'DB_Spaces',
	'DBA_ABMJobs',
	'asigna_permisos_sp',
...
union
(select name as 'Objetos' from master.dbo.sysobjects where name in
	(
	'aviso_shutdown',
	'sp_who2_dba'
	)
...
union
(select name as 'Objetos' from msdb.dbo.sysobjects where name in
	(
	'AUDITORIA'
	)
)
