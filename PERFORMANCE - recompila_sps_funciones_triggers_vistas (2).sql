...
while  @cont <> 0
  begin
    ...
    select 'select ''' + 'use ' + @dbname + ''''
    select 'select ''' + 'go' + ''''
    select @command = 'use ' + @dbname + ' exec (''select DISTINCT''''exec sp_refreshview ''''''''['''' + o.name + '''']'''''''''''' from sys.objects o INNER JOIN sys.sql_expression_dependencies AS sed ON o.object_id = sed.referencing_id where o.type = ''''V'''' '')'
    select @command
    select @cont = @cont - 1
  end
go