...
while  @cont <> 0
  begin
    ...
    select 'select ''' + 'use ' + @dbname + ''''
    select 'select ''' + 'go' + ''''
    select @command = 'use ' + @dbname + ' exec (''select ''''exec sp_recompile ''''''''['''' + u.name + ''''].['''' + o.name + '''']'''''''''''' from sysobjects o join sysusers u on o.uid = u.uid where o.type = ''''FN'''' and o.name not like ''''dt_%'''' order by o.name, u.name'')'
    select @command
    select @cont = @cont - 1
  end
go
...
while  @cont <> 0
  begin
    ...
    select 'select ''' + 'use ' + @dbname + ''''
    select 'select ''' + 'go' + ''''
    select @command = 'use ' + @dbname + ' exec (''select ''''exec sp_recompile ''''''''['''' + u.name + ''''].['''' + o.name + '''']'''''''''''' from sysobjects o join sysusers u on o.uid = u.uid where o.type = ''''TR'''' and o.name not like ''''dt_%'''' order by o.name, u.name'')'
    select @command
    select @cont = @cont - 1
  end
go
...
while  @cont <> 0
  begin
    ...
    select 'select ''' + 'use ' + @dbname + ''''
    select 'select ''' + 'go' + ''''
    select @command = 'use ' + @dbname + ' exec (''select DISTINCT''''exec sp_refreshview ''''''''['''' + o.name + '''']'''''''''''' from sys.objects o INNER JOIN sys.sql_dependencies AS sed ON o.object_id = sed.object_id where o.type = ''''V'''' '')'
    select @command
    select @cont = @cont - 1
  end
go